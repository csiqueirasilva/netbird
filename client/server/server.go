...
type Server struct {
	...
	// come back without it -- the login succeeds, the tunnel is then refused,
	// and nothing in either message points at the reason.
	//
...
func (s *Server) sendLogoutRequestWithConfig(ctx context.Context, config *profilemanager.Config) error {
	...
	mgmClient, err := mgm.NewClient(ctx, config.ManagementURL.Host, key, mgmTlsEnabled,
		mgm.WithClientCertificates(config.ClientCertKeyPairs))
	if err != nil {
