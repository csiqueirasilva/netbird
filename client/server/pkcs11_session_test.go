...
// A certificate already loaded, from the PIN in the environment on the
...
func TestPKCS11RestoreDoesNotOverwriteALoadedCertificate(t *testing.T) {
	...
	loaded := []tls.Certificate{{Certificate: [][]byte{{1, 1}}}}
	config := &profilemanager.Config{
		PKCS11:             profilemanager.PKCS11Config{ModulePath: "/usr/lib/libeTPkcs11.so"},
		ClientCertKeyPairs: loaded,
	}
	s.restorePKCS11Certificates(config)

	if string(config.ClientCertKeyPairs[0].Certificate[0]) != string(loaded[0].Certificate[0]) {
